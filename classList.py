...
class Piercer(Main_Character):
    def __init__(self, HP = 40, DEF = 0, minATK = 4, maxATK = 6, critChance = 8):
        ...
        self.blaster_explosion = 0
        self.multiple_glories = 5
    ...
    def shoot_attack(self):
        if die.randint(1, self.critChance) == 1:
            ...
            self.arrow_rain_cooldown -= 1 if self.arrow_rain_cooldown > 0 else 0
            return crit_hit
        else:
            print(f"Normal hit.")
            self.arrow_rain_cooldown -= 1 if self.arrow_rain_cooldown > 0 else 0
    ...
    def Arrow_Rain(self):
        if self.arrow_rain_cooldown > 0:
            print(f"Arrow Rain is on cooldown for {self.arrow_rain_cooldown} more turns.")
            return 0
        print("You used Arrow Rain! Dealt 5 damage to all enemies!")
        self.arrow_rain_cooldown = 3
        return 5
    ...
    def Arrow_Explosion(self):
        if self.blaster_explosion > 0:
            print(f"Blaster Explosion is on cooldown for {self.blaster_explosion} more turns.")
            return 0
        print("You used Blaster Explosion! Dealt 24 damage!")
        self.blaster_explosion = 4
        return 24
    ...
    def One_Shot_Multiple_Glories(self):
        if self.multiple_glories > 0:
            print(f"One Shot Multiple Glories is on cooldown for {self.multiple_glories} more turns.")
            return 0
